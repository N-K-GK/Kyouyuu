...
本課題では、GitHubの**共有リポジトリモデル（Shared Repository Model）**を使用し、3人で共同開発を行った。
...
今回の課題では、GitHubのWebブラウザとVisual Studio Code（VSCode）のGUI機能を使用して作業を行った。
...
今回の課題では、Aがリポジトリの管理者となり、BとCをCollaboratorとして追加した。
...
今回の課題では、A・B・Cそれぞれが作業ブランチを作成し、Pull Requestを作成して`main`へ変更を取り込んだ。
...
今回の課題では、A・B・Cの3人で**共有リポジトリモデル**による共同開発を行った。
...
CがPull Requestを作成し、Aがレビュー・Mergeすることで、指定された一連の課題が完了した。
...
この課題を通して、GitHubのリポジトリ管理、Collaborator管理、ブランチ作成、Commit、Push、Pull、Pull Request、レビュー、Mergeという一連の共同開発の流れを実践した。
